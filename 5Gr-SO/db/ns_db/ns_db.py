...
def get_aiml_info(nsId, problem):
    ...
    nsId: string
        Identifier of the Network Service Instance.
    problem: string
        Identifier of the handled AIML problem.
    Returns
    -------
    Dict
        Dictionary with the AIML information regarding the problem.
    """
    ns = ns_coll.find_one({"nsId": nsId})
    if "aiml_info" in ns:
        if problem in ns["aiml_info"]:
            return ns["aiml_info"][problem]
        else:
            return {}
    else:
        return {}
...
def set_aiml_info(nsId, problem, aiml_info):
    """
    Adds the aiml info associated to this NS instance.
    ----------
    nsId: string
        Identifier of the Network Service Instance.
    problem: string
        Identifier of the problem treated by aiml
    aiml_info: dict
        Dictionary whose key is the problem name and the content is the associated info.
    Returns
    -------
    """
    ns = ns_coll.find_one({"nsId": nsId})
    if "aiml_info" in ns:
        # this means that we need to create a new entry(new problem) or update the existing one
        aiml_dict = ns["aiml_info"]
        if problem not in aiml_dict:
            aiml_dict[problem] = {}
